 ...
 * 时间: 2020-12-18
 * 手机: Huawei P40Pro
 * 系统: Android 10 EMUI 10.1.0
 * 支付宝: 10.2.10 // 支付宝版本不一样很可能发生异常哦！
 * 无障碍模式: 打开
 * 截屏权限: 打开
 * Auto.js: Version 4.0.5 Alpha
 * VSCode: 1.51.1
 * 本软件版本: v1.0.0
 */
// 自定义一系列错误码
var RET_SUCCES = 0; // 成功返回
var RET_FAILED = -1; // 失败返回
var RET_ERR_AUTO_FAIED = -2; // 获取无障碍服务失败
var RET_ERR_GET_SCREENSHOT_PRI_FAILED = -3; // 获取截屏权限失败
...
function anti_forest_err_return (err) {
  switch (err) {
    ...
    case RET_ERR_INVALID_TIME_INTERVAL:
      DUMP_OUT_ERR("设置蚂蚁森林重试时间间隔设置有误！");
      break;
    case RET_ERR_INVALID_RETRY_TIMES:
      DUMP_OUT_ERR("设置重试打开蚂蚁森林次数有误！");
      break;
...
function ant_forest_enter (time_interval, retry_times) {
  ...
  for (index = 0; index <= retry_times; index++) {
    /* 通过下面两个"控件"来判断成功进入了蚂蚁森林 */
    var close_exist = className("android.widget.TextView").desc("关闭").exists();
    // 4.0.5Alpha只要使用id("com.android.systemui:id/xxx...")的时候会报错
    // var forest_title_exist = id("com.alipay.mobile.nebula:id/h5_tv_title").exists();
    var forest_title_exist = idMatches("h5_tv_title");
    if (close_exist && forest_title_exist) {
      enter_flag = 0;
      break;
    } else if (index) {
      debug_str = "第 " + index + "/" + retry_times + " 次重新进入蚂蚁森林.\n    头部关闭是否存在: " + close_exist + ", 标题是否存在: " + forest_title_exist;
      DUMP_OUT_DEBUG(debug_str);
    }
    if (0 == index % 5) {
      /* 如何获取data(支付宝客户端的标准scheme)
      * http://myjsapi.alipay.com/jsapi/h5app-lifecycle.html
      * https://www.jianshu.com/p/5ea0b9d84caa
      * 获取得到蚂蚁森林的scheme("URL"): alipay://platformapi/startapp?appId=60000002
      * packageName:通过Auto.js的 app.getPackageName('支付宝') 获得
      */
      app.startActivity({
...
function startup () {
  ...
  DUMP_OUT_DEBUG("获取设备尺寸 (" + device.width + ", " + device.height + ")");
